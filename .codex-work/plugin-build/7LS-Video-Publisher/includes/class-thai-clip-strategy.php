 ...
 * for the "คลิปไทย" content mode.
 */
class Thai_Clip_Strategy implements Mode_Strategy {
    ...
    public function get_default_params(): array {
        return ['type' => 'thai_clip'];
    }

    public function map_fields(array $raw): array {
        // Unwrap if nested inside a 'video' key
        ...
        return [
            ...
            'categories'     => $this->normalize_terms($raw['categories'] ?? $raw['category'] ?? $raw['categorys'] ?? []),
            'tags'           => $this->normalize_terms($raw['tags'] ?? []),
            'actors'         => $this->normalize_terms($raw['actors'] ?? $raw['actor'] ?? $raw['uploader'] ?? []),
            'extra_meta'     => [
                'source'   => sanitize_text_field($raw['source'] ?? ''),
                'uploader' => sanitize_text_field($raw['uploader_name'] ?? $raw['uploaderName'] ?? ''),
            ],
    ...
    public function validate(array $raw): bool|\WP_Error {
        ...
        if (empty($title)) {
            return new \WP_Error('missing_title', 'Video title is required');
        }
